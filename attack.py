...
def PRG_attack(IV, x):
    """ Given an Initial Value [list of 256 bytes] to the Cipher `IV` and a byte stream `x` [list of bytes of variable length], return 1 if the `x` is random, and 0 if pseudorandom (generated by the stream cipher)."""
    length = len(x)
...
def EAV_attack(m0, m1, c):
    """ Given the two messages selected by you, and the ciphertext c, return 0 if the ciphertext is an encryption of the message m0, and 1 if the ciphertext is an encryption of the message m1 """
    # Your code here!
    IV = c[0:256]
    cipher = c[256:]
